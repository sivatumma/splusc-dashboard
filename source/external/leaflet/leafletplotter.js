L.Polyline.plotter = L.Polyline.extend({
    ...
    _addNewMarker: function(e) {
        var newMarker = this._getNewMarker(e.latlng, {
            icon: this._editIcon
        });
    ...
    _redrawHalfwayPoints: function() {
        ...
        for (index in this._lineMarkers) {
            ...
            var halfwayMarker = new L.Marker([
                (this._lineMarkers[index].getLatLng().lat + this._lineMarkers[index + 1].getLatLng().lat) / 2, (this._lineMarkers[index].getLatLng().lng + this._lineMarkers[index + 1].getLatLng().lng) / 2,
            ], {
                icon: this._editIcon,
                // opacity: 0.5
                opacity: 0
